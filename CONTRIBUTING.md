...
- `tests/integration` — against real Pinecone/OpenAI, requires `PINECONE_API_KEY`/`LLM__API_KEY` in `.env` (see `tests/integration/conftest.py`); skips with a clear reason if they're missing.
