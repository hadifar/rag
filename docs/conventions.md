...
## Adding a new observability or checkpointer backend: extend the `Settings`-picked branch

Both `adapters/observability.py`'s `open_trace_config` and `adapters/checkpointer.py`'s
`open_checkpointer` pick their concrete implementation from a `Settings` field
(`OBSERVABILITY_BACKEND`, `CHECKPOINTER_BACKEND`) inside one async context manager, so callers
never branch on backend themselves — they just call `trace_config(name)` or use the checkpointer
object. A new backend is a new branch inside that one function, validated in `config.py`, not a
new code path exposed to callers.
